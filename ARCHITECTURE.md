...
El protocolo garden incorpora `stream: true` y transmite las cabeceras Range al lector local para reproducción multimedia. AudioEngine usa un elemento HTMLAudioElement local y bajo demanda para la canción; los efectos breves siguen en Web Audio a menor volumen. Un contador de solicitudes evita reactivar audio tras silenciar durante una carga pendiente. Al ocultar la pestaña se pausa el archivo y se exige reactivación explícita.
